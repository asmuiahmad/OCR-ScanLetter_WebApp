...
from config.ocr_utils import (
    ...
    extract_isi_suratkeluar,
    extract_jam,
    extract_penerima_surat_masuk,
    extract_pengirim,
    extract_roman_numeral,
    extract_tanggal_acara,
    extract_tempat,
...
def clean_letter_number(raw_text):
    ...
    if re.search(r"[A-Z]+.*[0-9]+.*/", cleaned, re.IGNORECASE) or re.search(
        r"[0-9]+.*[A-Z]+.*/", cleaned, re.IGNORECASE
    ):
...
def extract_ocr_data_surat_masuk(file_path):
    try:
        ...
        for line in lines:
            ...
            if match:
                ...
                if cleaned_nomor and cleaned_nomor != "Not found":
                    ...
                    logger.info(
                        f"Extracted and cleaned nomor_surat: {nomor_suratMasuk}"
                    )
                    break
        ...
        if (
            tanggal_str
            and isinstance(tanggal_str, str)
            and re.match(r"\d{4}-\d{2}-\d{2}", tanggal_str)
        ):
        ...
        isi_surat = (
            improve_text_spacing(isi_surat_raw) if isi_surat_raw else isi_surat_raw
        )
    ...
    try:
        ...
        for item in results:
            try:
                ...
                saved_count += 1
                logger.info(
                    f"Saved document to database: {item.get('nomor_surat', 'Not found')}"
                )
            ...
            for file in files:
                ...
                try:
                    ...
                    if not allowed_file(filename):
                        flash(
                            f"File '{filename}' tidak diizinkan. Hanya file dengan ekstensi .png, .jpg, .jpeg, .webp, .tiff, .bmp yang diizinkan.",
                            "error",
                        )
                        continue
    ...
    try:
        ...
        for item in data:
            try:
                ...
                if tgl_acara:
                    try:
                        tanggal_acara_obj = datetime.strptime(
                            tgl_acara, "%Y-%m-%d"
                        ).date()
                    except Exception:
                        try:
                ...
                surat_masuk = SuratMasuk(
                    ...
                    status_suratMasuk="pending",
                    initial_nomor_suratMasuk=item.get(
                        "full_letter_number", "Not found"
                    ),
                    initial_pengirim_suratMasuk=item.get(
                        "pengirim_suratMasuk", "Not found"
                    ),
                    initial_penerima_suratMasuk=item.get(
                        "penerima_suratMasuk", "Not found"
                    ),
