    ...
    if clean_endpoint in route_breadcrumbs:
        ...
        if clean_endpoint.startswith("edit_"):
            ...
            if parent_endpoint in route_breadcrumbs:
                breadcrumbs.append(
                    (
                        route_breadcrumbs[parent_endpoint],
                        url_for(f"main.{parent_endpoint}"),
                    )
                )
        current_url = (
            url_for(endpoint, **view_args) if endpoint != "main.index" else None
        )
                    ...
                    notification_items.append(
                        {
                            ...
                            or "Pengirim tidak diketahui",
                            "penerima": surat.penerima_suratMasuk
                            or "Penerima tidak diketahui",
                            "nomor": surat.nomor_suratMasuk or "Nomor tidak tersedia",
                            "tanggal_display": surat.tanggal_suratMasuk.strftime(
                                "%d %b %Y"
                            )
                            if surat.tanggal_suratMasuk
                            else "Tanggal tidak diketahui",
                            "created_at_display": surat.created_at.strftime(
                                "%d %b %Y %H:%M"
                            )
                            if surat.created_at
                            else "",
                    ...
                    notification_items.append(
                        {
                            ...
                            "tanggal_display": surat.tanggal_suratKeluar.strftime(
                                "%d %b %Y"
                            )
                            ...
                            and len(surat.isi_suratKeluar) > 100
                            else (surat.isi_suratKeluar or ""),
                        }
                    )

                notification_items.sort(
                    key=lambda item: item.get("created_at_sort") or "", reverse=True
                )
