...
def test_ocr_enhancement():
    ...
    try:
        ...
        if not test_text:
            return jsonify(
                {"success": False, "message": "No text provided for testing"}
            ), 400
        ...
        enhanced_text = surat_masuk_enhancer.enhance_surat_masuk_text(
            cleaned_text, field_type
        )
        ...
        return jsonify(
            {
                ...
                "improvements": {
                    ...
                    "word_count_change": len(enhanced_text.split())
                    - len(test_text.split()),
                },
            }
        )
        ...
        return jsonify(
            {"success": False, "message": f"Error testing OCR enhancement: {str(e)}"}
        ), 500
